...
class HotelBookingHistoryController extends GetxController {
  final DataController dataController = Get.put(DataController());
  var isLoading = false.obs;
  var bookingHistoryModel = HotelBookingHistoryModel().obs;
  var bookingHistoryList = <HotelBookingHistoryModel>[].obs;
  ...
  Future<void> fetchBookings() async {
    ...
    try {
      ...
      var response = await http.post(
          Uri.parse(
              '${baseURL}api/HotelBooking/myBookings?pageSize=100&pageNumber=1'),
          headers: headers,
          body: body);
      // print("This is my Token: ${dataController.myToken.value}");

      var jsonData = json.decode(response.body);
      List<HotelBookingHistoryModel> data = (jsonData as List)
          .map((item) => HotelBookingHistoryModel.fromJson(item))
          .toList();
